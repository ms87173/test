...
import { questionnaireActions, signAndSubmitActions } from '../../store/actions';
...
export class FormlyFieldsService {
  ...
  getFormlyLifeCycleEventByQuestionType(questionType: string, requestId: string,
    workflowId: string, taskId: string, taskType: string = TASK_TYPES.QUESTION): FormlyLifeCycleOptions {
    switch (questionType) {
          ...
          onInit: (form?: FormGroup, field?: FormlyFieldConfig, model?: any, options?: FormlyFormOptions) => {
            ...
            formObj.valueChanges
              ...
              .subscribe((fieldValue) => {
                ...
                if (taskType === TASK_TYPES.SIGN_AND_SUMBIT) {
                  this.store.dispatch(new signAndSubmitActions.SetSignAndSubmitId(key));
                  this.store.dispatch(new signAndSubmitActions.GetSignAndSubmitDelta(questionaireRequest));
